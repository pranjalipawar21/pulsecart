...
export default function SuppliersTab({ apiFetch, T }) {
  ...
  const getAIReview = async () => {
    setLoading(true);
    const prompt = `You are a procurement AI. Based on REAL supplier performance:
${supplierList.map(s => `- ${s.name}: Reliability ${s.reliability_score}, Quality ${s.quality_score}/10, Defect Rate ${s.defect_rate}%`).join('\n')}
Give 3 specific procurement recommendations. Plain text, max 80 words.`;
    ...
    <div style={{ padding: '20px', display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '20px' }}>
      <div style={{ background: T.panel, padding: '24px', borderRadius: '12px', border: `1px solid ${T.border}` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h3 style={{ fontSize: 18, fontWeight: 700 }}>Supplier Scorecards</h3>
          <span style={{ fontSize: 10, background: T.success+'22', color: T.success, padding: '2px 8px', borderRadius: 10, fontWeight: 700 }}>SQL DRIVEN</span>
        </div>

        {fetching ? (
          <div style={{ padding: 40, textAlign: 'center', color: T.muted }}>Loading suppliers...</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {supplierList.length === 0 && (
                <tr><td colSpan="5" style={{ padding: 20, textAlign: 'center', color: T.muted }}>No suppliers found in database.</td></tr>
              )}
              {supplierList.map(s => (
                <tr key={s.id} style={{ borderBottom: `1px solid ${T.dimmed}`, fontSize: 13 }}>
                  ...
                  <td style={{ padding: 12 }}>
                    <div style={{ fontWeight: 700, color: (s.reliability_score || 0) > 85 ? T.success : T.brandAlt }}>{s.reliability_score || 0}%</div>
                    <div style={{ fontSize: 9, color: T.muted }}>{s.avg_delivery_days || 0}d avg</div>
                  </td>
                  <td style={{ padding: 12 }}>{s.quality_score || 0}/10 ★</td>
                  <td style={{ padding: 12, color: (s.defect_rate || 0) > 2 ? T.danger : T.text }}>{s.defect_rate || 0}%</td>
                  <td style={{ padding: 12 }}>
                     <span style={{ fontSize: 10, padding: '2px 6px', borderRadius: 4, background: (s.reliability_score || 0) > 80 ? T.success+'15' : T.danger+'15', color: (s.reliability_score || 0) > 80 ? T.success : T.danger }}>
                       {(s.reliability_score || 0) > 80 ? 'Verified' : 'Review Needed'}
                     </span>
                  </td>
                </tr>
              ))}
        ...
        <button 
          onClick={getAIReview} 
          disabled={loading || supplierList.length === 0} 
